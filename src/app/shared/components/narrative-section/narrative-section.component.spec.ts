...
describe('NarrativeSectionComponent', () => {
  ...
  it('renders narrative__pending when isPending is true', () => {
    ...
    expect(el.querySelector('.narrative__para')).toBeNull();
  });
  ...
  it('renders @for paragraphs when isPending is false', () => {
    ...
    expect(paras[0].textContent).toContain('First paragraph.');
    expect(paras[1].textContent).toContain('Second paragraph.');
    expect(el.querySelector('.narrative__pending')).toBeNull();
  });

  it('always renders the caveat <p> regardless of isPending state', () => {
    // When pending
    component.ship = { ...baseShip, narrative: [] };
    fixture.detectChanges();
    ...
    // When not pending
    component.ship = { ...baseShip, narrative: ['Real content.'] };
    fixture.detectChanges();
