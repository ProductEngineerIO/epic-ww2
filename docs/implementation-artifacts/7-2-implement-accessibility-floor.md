...
- [ ] In `src/app/shared/components/ship-hero/ship-hero.component.html`, add `aria-hidden="true"` to both `<figcaption>` elements (both the normal and error-state branches)
  ```html
...
- [ ] In `src/app/shared/components/ship-hero/ship-hero.component.ts`, add a getter that derives a meaningful alt string if `ship.altText` is still the placeholder value:
  ```typescript
...
- [ ] Confirm `aria-hidden="true"` was applied to ShipHero's `<figcaption>` (Task 1) to prevent double announcement
- [ ] Verify the heading hierarchy in WitnessTrioBlock: NarrativeSection has `<h2 id="narrative-heading">Where was it going</h2>` — this is the only `<h2>` under the `<h1>`, which is correct ✅

### Task 6 — Semantic HTML audit: HomeComponent and FleetGrid (AC: 1, 2, 4, 5)
**Prerequisite: Stories 5.1, 5.2, 5.3 must be complete.**
- [ ] Verify `HomeComponent` has `<h1>` for the site/page title (per Story 5.3 AC)
- [ ] Verify `FleetGrid` thumbnails use `[alt]="ship.name + ' — thumbnail'"` — NOT `ship.altText`
  - `ship.altText` is the FULL attribution format for the hero image; thumbnails need the shorter form
  - Example: `alt="USS Valley Forge — thumbnail"`
- [ ] Verify `FleetGrid` grid cell links have touch targets ≥44×44px (each `<a>` element)
- [ ] Verify `FleetGrid` has `@media (prefers-reduced-motion: reduce)` disabling hover scale on thumbnails
- [ ] Verify `HomepageHero` uses `[alt]="ship.name + ', photographed by Howard Hertzog'"` — **NOT** `ship.altText` (different format: no "San Francisco Bay, c. 1944–1946" suffix for homepage editorial choice)

### Task 7 — Semantic HTML audit: AboutComponent and NotFoundComponent (AC: 2)
**Prerequisite: Stories 6.1, 6.2 must be complete.**
- [ ] Verify `AboutComponent` / `ComingSoon` has proper heading hierarchy:
  - The "Coming soon" heading must be `<h1>` (it is the primary heading of the page)
  - Story 6.2 specified `<h2>` for NotFoundComponent's "Page not found." heading — **this must be changed to `<h1>`** for WCAG compliance; there is no preceding `<h1>` on the not-found page
  - Update `ComingSoon` component (or its hosting page) to use `<h1>` not `<h2>` as the page-level heading
- [ ] Verify NotFoundComponent's "Back to fleet" link has a descriptive accessible label (either via text content or `aria-label`)

### Task 8 — Color contrast audit (AC: 7)
- [ ] Open `src/styles/_tokens.scss` and check these token pairs using a contrast ratio checker (e.g., browser DevTools → Accessibility panel, or https://webaim.org/resources/contrastchecker/):

  | Token (foreground) | Value | Background token | Value | Use case | Required ratio |
  |---|---|---|---|---|---|
  | `--color-on-surface` | `#ede9df` | `--color-bg` | `#16160e` | Body text | 4.5:1 ✓ (expect ~15:1) |
  | `--color-khaki` | `#c9b87a` | `--color-bg` | `#16160e` | Ship names, headings | 3:1 (large) |
  | `--color-steel` | `#8a9aaa` | `--color-surface` | `#23231a` | Secondary text | 4.5:1 |
  | **`--color-steel-muted`** | **`#5a6870`** | **`--color-surface-raised`** | **`#2e2e22`** | **"Not confirmed" placeholder** | **4.5:1 ⚠️ likely FAILS** |
  | `--color-olive` | `#7a8c44` | `--color-bg` | `#16160e` | Active nav, focus ring color | 3:1 (UI components) |
  | `--color-on-surface` | `#ede9df` | `--color-overlay` | `rgba(22,22,14,.82)` | Attribution caption over hero | 4.5:1 |

- [ ] **If `--color-steel-muted` fails 4.5:1 on `--color-surface-raised`:** Update the token in `_tokens.scss` to a lighter value that passes. The "Not confirmed" text is informational (not decorative), so it must meet 4.5:1. Suggested fix: `#7a8e9a` (a lighter steel-muted that preserves the muted aesthetic).
- [ ] Document contrast results in the Dev Agent Record below.

### Task 9 — Final accessibility sweep (AC: 1–7)
- [ ] Run `ng serve` and verify site end-to-end with keyboard navigation:
  - Tab through PersistentNav → FleetGrid → Fleet thumbnails → Ship page → ShipNav → focus ring visible on all
  - Arrow keys on a ship page route ShipNav correctly ✅ (ShipNavComponent)
  - Open ChromeVox or macOS VoiceOver and navigate to a ship page — verify: (a) `<h1>` announces ship name once, (b) `<h2>` announces "Where was it going", (c) attribution caption NOT announced from figcaption (aria-hidden), (d) image alt text announced with full attribution format
- [ ] Verify no `outline: none` appears anywhere in the SCSS without a custom focus replacement:
  ```bash
